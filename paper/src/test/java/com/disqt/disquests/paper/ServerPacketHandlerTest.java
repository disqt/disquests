...
class ServerPacketHandlerTest {
    ...
    void setUp() {
        ...
        bukkitMock.when(Bukkit::getOnlinePlayers).thenReturn(
                List.of(ownerPlayer, contributorPlayer, bystanderPlayer));
    ...
    void saveExistingQuest_onlyNotifiesOwnerAndContributors() {
        ...
        QuestData quest = new QuestData(
                questId, "Test Quest", "Original content",
                OWNER, "Owner", Visibility.OPEN,
                List.of(), 1000L, null, false, null, null);
        dm.saveQuest(quest);
        dm.addContributor(questId, CONTRIBUTOR, true);

        byte[] packet = PacketCodec.writeSaveQuest(
                questId, "Updated Title", "Updated content",
        ...
        verify(contributorPlayer, atLeastOnce())
                .sendPluginMessage(eq(plugin), eq(CHANNEL), any(byte[].class));
        verify(bystanderPlayer, never())
                .sendPluginMessage(any(), any(), any());
    }
}
