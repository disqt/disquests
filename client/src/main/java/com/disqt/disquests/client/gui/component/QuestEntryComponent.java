...
public class QuestEntryComponent extends BaseUIComponent {
  ...
  public QuestEntryComponent(Quest quest) {
    ...
    LOGGER.debug(
        "Created QuestEntryComponent: quest={}, owner={}, isOwned={}, visibility={}, pinned={}",
        quest.getTitle(),
        quest.getOwnerName(),
        isOwnedByPlayer,
        quest.getVisibility(),
        ClientSession.isPinned(quest.getId()));
    ...
    if (LOGGER.isDebugEnabled()) {
      this.mouseDown()
          .subscribe(
              (click, doubled) -> {
                LOGGER.debug(
                    "[EVENT mouseDown] quest={}, click=({}, {}), doubled={}",
                    quest.getTitle(),
                    click.x(),
                    click.y(),
                    doubled);
  ...
  public void draw(OwoUIGraphics context, int mouseX, int mouseY, float partialTicks, float delta) {
    ...
    if (selected) {
      context.fill(
          entryX, entryY, entryX + entryWidth, entryY + ENTRY_HEIGHT, Colors.ENTRY_SELECTED);
    } else if (hovered) {
    ...
    if (selected && theme == Theme.INSET) {
      ...
      context.fill(
          entryX,
          entryY + ENTRY_HEIGHT - 1,
          entryX + entryWidth,
          entryY + ENTRY_HEIGHT,
          0xFF2A2A2A);
      context.fill(
          entryX + entryWidth - 1, entryY, entryX + entryWidth, entryY + ENTRY_HEIGHT, 0xFF2A2A2A);
    }
  ...
  private static final org.slf4j.Logger LOGGER =
      org.slf4j.LoggerFactory.getLogger("Disquests.QuestEntry");
  private static final long DOUBLE_CLICK_MS = 300;
  private static java.lang.ref.WeakReference<QuestEntryComponent> lastClickedEntry =
  ...
  public boolean onMouseDown(Click click, boolean doubled) {
    ...
    double relY = click.y();
    LOGGER.debug(
        "  relX={}, relY={}, width={}, pinArea=[{}-{}, 12-26], isPinned={}",
        relX,
        relY,
        this.width(),
        this.width() - 20,
    ...
    if (relX >= this.width() - 20 && relX <= this.width() && relY >= 12 && relY <= 26) {
      if (!isOwnedByPlayer && !isContributor) {
        LOGGER.debug(
            "  -> PIN CLICK blocked: player is not owner or contributor of {}", quest.getTitle());
        return true;
      }
    ...
    boolean isDoubleClick =
        lastClickedEntry.get() == this && (now - lastClickTime) < DOUBLE_CLICK_MS;
