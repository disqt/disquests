...
/**
 * Registers Fabric screen event hooks that log all mouse interactions on Disquests screens at DEBUG
 * level. Enable by setting the "Disquests" logger to DEBUG in log4j2 config.
 *
 * <p>Registered once at mod init. Only fires for Disquests screens.
 */
  ...
  public static void register() {
    ScreenEvents.AFTER_INIT.register(
        (client, screen, width, height) -> {
          ...
          ScreenMouseEvents.beforeMouseRelease(screen)
              .register(
                  (scr, click) -> {
                    LOGGER.debug(
                        "[RELEASE PRE] {} at ({}, {}) button={}",
                        screenName,
                        click.x(),
                        click.y(),
                        click.button());
          ...
          ScreenMouseEvents.beforeMouseScroll(screen)
              .register(
                  (scr, mouseX, mouseY, hAmount, vAmount) -> {
                    LOGGER.debug(
                        "[SCROLL] {} at ({}, {}) h={} v={}",
                        screenName,
                        mouseX,
