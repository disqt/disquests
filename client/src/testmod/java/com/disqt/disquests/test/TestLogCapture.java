...
/**
 * Captures Log4j2 log events for assertion in tests. Usage: TestLogCapture capture =
 * TestLogCapture.attach("Disquests/QuestEntry"); // ... do stuff that logs ... List<String>
 * messages = capture.getMessages(); capture.detach();
 */
public class TestLogCapture extends AbstractAppender {
  ...
  public boolean hasMessageContaining(String substring) {
    return events.stream().anyMatch(e -> e.getMessage().getFormattedMessage().contains(substring));
  }
