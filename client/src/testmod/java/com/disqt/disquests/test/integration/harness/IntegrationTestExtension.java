...
public class IntegrationTestExtension
    implements ExecutionCondition, BeforeAllCallback, BeforeEachCallback, ParameterResolver {
  ...
  public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext ctx) {
    ...
    if (hasPlayerA && isA)
      return ConditionEvaluationResult.enabled("PlayerA method on PlayerA client");
    if (hasPlayerB && !isA)
      return ConditionEvaluationResult.enabled("PlayerB method on PlayerB client");

    return ConditionEvaluationResult.disabled(
        "Skipped: method is @"
            + (hasPlayerA ? "PlayerA" : "PlayerB")
            + " but this client is "
            + TestContext.getPlayerRole());
  }
