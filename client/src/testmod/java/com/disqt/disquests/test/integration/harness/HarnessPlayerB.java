...
public class HarnessPlayerB implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        if (shouldSkip("HarnessPlayerB")) return;
        ...
        if (harness) {
            ...
            try {
                context.waitFor(client -> Files.exists(PhaseSync.getSyncDir().resolve("player-a-done.done")), seconds(15));
    ...
    private void harnessLoop(ClientGameTestContext context) {
        ...
        while (true) {
            try { Files.deleteIfExists(syncDir.resolve("results-b.txt")); } catch (IOException ignored) {}
            try { Files.deleteIfExists(syncDir.resolve("error-playerb.done")); } catch (IOException ignored) {}

            PhaseSync.signal("client-b-ready");
    ...
    private void writeResult(String result) {
        try {
            ...
            Files.writeString(syncDir.resolve("results-b.txt"), result);
        } catch (IOException e) {
