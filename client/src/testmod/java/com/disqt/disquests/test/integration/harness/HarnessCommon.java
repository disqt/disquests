...
public class HarnessCommon {
  ...
  private static final String TESTS_PACKAGE =
      System.getProperty("disquests.test.package", "com.disqt.disquests.test.integration.journeys");
  ...
  private static void harnessLoop(ClientGameTestContext context, String role) {
    ...
    while (true) {
      try {
        Files.deleteIfExists(syncDir.resolve("results-" + role + ".txt"));
      } catch (IOException ignored) {
      }
      try {
        Files.deleteIfExists(syncDir.resolve("error-player" + role + ".done"));
      } catch (IOException ignored) {
      }
