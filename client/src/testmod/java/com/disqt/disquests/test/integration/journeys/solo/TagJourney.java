...
class TagJourney {
    ...
    /**
     * Count the number of tags currently shown in the tag-editor FlowLayout.
     * Each tag occupies two children: a label and an "x" button. The final child
     * is the "+ Tag" button (present when below MAX_TAGS). So tag count = (children.size() - 1) / 2.
     * Note: this formula is only correct below MAX_TAGS (8); tests stay well below that.
     */
    ...
    /**
     * Click the first predefined tag button in TagPickerScreen.
     * Predefined buttons are children of "predefined-list".
     */
    private void clickFirstPredefinedTag(ClientGameTestContext context) {
        ...
        double[] pos = context.computeOnClient(c -> {
            ...
            return new double[]{first.x() + first.width() / 2.0, first.y() + first.height() / 2.0};
        });
    ...
    private void clickAddTagButton(ClientGameTestContext context) {
        ...
        double[] pos = context.computeOnClient(c -> {
            ...
            return new double[]{addBtn.x() + addBtn.width() / 2.0, addBtn.y() + addBtn.height() / 2.0};
        });
    ...
    void addPredefinedTag(ClientGameTestContext context) {
        ...
        then("returns to edit mode with one tag in tag-editor");
            ...
            org.junit.jupiter.api.Assertions.assertTrue(count >= 1,
                    "Expected at least 1 tag in tag-editor, got: " + count);
    }
    ...
    void saveAndVerifyTagInViewMode(ClientGameTestContext context) {
        ...
        then("view mode shows the tag in tag-display");
            ...
            org.junit.jupiter.api.Assertions.assertEquals(1, tagCount,
                    "Expected 1 tag in tag-display, got: " + tagCount);
    }
    ...
    void tagSearchFilter(ClientGameTestContext context) {
        ...
        String tagName = context.computeOnClient(c ->
    ...
    void addCustomTag(ClientGameTestContext context) {
        ...
        then("edit mode shows the custom tag");
            ...
            context.waitFor(client -> {
                ...
                return tagEditor.children().stream().anyMatch(child -> {
    ...
    /**
     * Click the "Add" button in the custom-row of TagPickerScreen.
     * It is the last child of custom-row.
     */
    ...
    void removeTag(ClientGameTestContext context) {
        ...
        org.junit.jupiter.api.Assertions.assertTrue(countBefore > 0,
                "Expected at least 1 tag before removal, got: " + countBefore);
        ...
        then("one fewer tag appears in tag-editor");
            ...
            org.junit.jupiter.api.Assertions.assertEquals(expectedAfter, countAfter,
                    "Expected " + expectedAfter + " tags after removal, got: " + countAfter);
    }

    /**
     * Click the first "x" (remove) button for the first tag in tag-editor.
     * Layout: [tagLabel, xBtn, tagLabel, xBtn, ..., +TagBtn]
     * The first "x" button is at index 1.
     */
