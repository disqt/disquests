...
class TagJourney {
  ...
  private void waitForViewMode(ClientGameTestContext context) {
    context.waitFor(
        client -> {
          if (!(client.gui.screen() instanceof DisquestsBaseScreen dScreen)) return false;
  ...
  private int tagEditorTagCount(ClientGameTestContext context) {
    return context.computeOnClient(
        c -> {
          if (!(c.gui.screen() instanceof DisquestsBaseScreen dScreen)) return 0;
          var root = dScreen.getRootComponent();
  ...
  private void clickFirstPickerChip(ClientGameTestContext context) {
    ...
    double[] pos =
        context.computeOnClient(
            c -> {
              if (!(c.gui.screen() instanceof DisquestsBaseScreen dScreen)) return null;
              var root = dScreen.getRootComponent();
  ...
  void hashtagIndependentSearch(ClientGameTestContext context) {
    ...
    boolean found =
        context.computeOnClient(
            c -> {
              if (!(c.gui.screen() instanceof DisquestsBaseScreen dScreen)) return false;
              var root = dScreen.getRootComponent();
              if (root == null) return false;
              FlowLayout chipCloud = root.childById(FlowLayout.class, "chip-cloud");
  ...
  private int tagEditorTagCount(net.minecraft.client.Minecraft client) {
    if (!(client.gui.screen() instanceof DisquestsBaseScreen dScreen)) return -1;
    var root = dScreen.getRootComponent();
