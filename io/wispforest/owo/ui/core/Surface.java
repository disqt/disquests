...
public interface Surface {
    ...
    static Surface tooltip(@Nullable class_2960 texture) {
        return (context, component) -> {
            class_8002.method_47946(context, component.x() + 4, component.y() + 4, component.width() - 8, component.height() - 8, texture);
        };
    }

    static Surface blur(float quality, float size) {
        return (context, component) -> {
            context.field_59826.method_70919(new BlurQuadElementRenderState(
                new Matrix3x2f(context.method_51448()),
                new class_8030(component.x(), component.y(), component.width(), component.height()),
    ...
    static Surface vanillaPanorama(boolean alwaysVisible) {
        return panorama(class_310.method_1551().field_1773.method_71649(), alwaysVisible);
    }

    static Surface panorama(class_766 renderer, boolean alwaysVisible) {
        return (context, component) -> {
            if (!alwaysVisible && class_310.method_1551().field_1687 != null) return;
            context.field_59826.method_70922(new CubeMapElementRenderState(
                renderer, true,
                new class_8030(component.x(), component.y(), component.width(), component.height()),
                context.field_44659.method_70863()
            ));
    ...
    static Surface tiled(class_2960 texture, int textureWidth, int textureHeight) {
        return (context, component) -> {
            context.method_25290(class_10799.field_56883, texture, component.x(), component.y(), 0, 0, component.width(), component.height(), textureWidth, textureHeight);
        };
    }
