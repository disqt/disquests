...
public interface ParentUIComponent extends UIComponent {
    ...
    @Override
    default boolean onMouseDown(class_11909 click, boolean doubled) {
        var iter = this.children().listIterator(this.children().size());

        while (iter.hasPrevious()) {
            var child = iter.previous();
            if (!child.isInBoundingBox(this.x() + click.comp_4798(), this.y() + click.comp_4799())) continue;
            if (child.onMouseDown(new class_11909(this.x() + click.comp_4798() - child.x(), this.y() + click.comp_4799() - child.y(), click.comp_4800()), doubled)) {
                return true;
            }
        }
    ...
    default class_5250 inspectorDescriptor() {
        ...
        return UIComponent.super.inspectorDescriptor().method_10852(
                class_2561.method_43470(" >" + padding.top() + "," + padding.bottom() + "," + padding.left() + "," + padding.right() + "<")
