...
public record Color(float red, float green, float blue, float alpha) implements Animatable<Color> {
    ...
    private static final Map<String, Color> NAMED_TEXT_COLORS = Stream.of(class_124.values())
            .filter(class_124::method_543)
            .collect(ImmutableMap.toImmutableMap(formatting -> {
                return formatting.method_537().toLowerCase(Locale.ROOT).replace("_", "-");
            }, Color::ofFormatting));
    ...
    public Color interpolate(Color next, float delta) {
        return new Color(
                class_3532.method_16439(delta, this.red, next.red),
                class_3532.method_16439(delta, this.green, next.green),
                class_3532.method_16439(delta, this.blue, next.blue),
                class_3532.method_16439(delta, this.alpha, next.alpha)
        );
    }
