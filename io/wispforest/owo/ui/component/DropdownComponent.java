...
public class DropdownComponent extends FlowLayout {
    ...
    protected static final class_2960 ICONS_TEXTURE = Owo.id("textures/gui/dropdown_icons.png");
     ...
     */
    public static <R extends ParentUIComponent> DropdownComponent openContextMenu(class_437 screen, R rootComponent, BiConsumer<R, DropdownComponent> mountFunction, double mouseX, double mouseY, Consumer<DropdownComponent> builder) {
        ...
        ScreenMouseEvents.beforeMouseClick(screen).register((screen_, click) -> {
            if (dismounted.isTrue() || dropdown.isInBoundingBox(click.comp_4798(), click.comp_4799())) return;
    ...
    public DropdownComponent text(class_2561 text) {
        this.entries.child(UIComponents.label(text).color(Color.ofFormatting(class_124.field_1080)).margins(Insets.of(2)));
        return this;
    }
            ...
            switch (entry.getNodeName()) {
                ...
                case "nested" -> {
                    var text = entry.getAttribute("translate").equals("true")
                            ? class_2561.method_43471(entry.getAttribute("name"))
                            : class_2561.method_43470(entry.getAttribute("name"));
    ...
    protected static void drawIconFromTexture(OwoUIGraphics context, ParentUIComponent dropdown, int y, int u, int v) {
        context.method_25290(class_10799.field_56883, ICONS_TEXTURE,
