...
public final class ModDataLoader {
    ...
    public static void load(ModDataConsumer consumer) {
        Map<class_2960, JsonObject> foundFiles = new HashMap<>();
            ...
            try (var stream = Files.walk(targetPath)) {
                stream.forEach(path -> {
                    ...
                    try {
                        ...
                        foundFiles.put(class_2960.method_60655(namespace, FilenameUtils.removeExtension(targetPath.relativize(path).toString())), GSON.fromJson(tabData, JsonObject.class));
                    } catch (IOException e) {
