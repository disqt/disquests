...
public class EntityWidget extends LeafInstanceWidget {
    ...
    public EntityWidget(double scale, class_1297 entity, @Nullable WidgetSetupCallback<EntityWidget> setupCallback) {
        this.scale = scale;
        this.entity = entity;
    ...
    public static class Instance extends LeafWidgetInstance<EntityWidget> {
        ...
        protected void doLayout(Constraints constraints) {
            ...
            if (this.widget.scaleToFit) {
                this.baseScale = Math.min(
                    this.transform.width() / this.widget.entity.method_17681(),
                    this.transform.height() / this.widget.entity.method_17682()
                ) * .6;
            }
        }
        ...
        public void draw(BraidGraphics graphics) {
            ...
            entityTransform.translate(0, -entity.method_17682() / 2, 0);

            var xRotation = 0f;
            var yRotation = 0f;

            var lastHeadYaw = entity instanceof class_1309 living ? living.field_6259 : 0;
            var lastYaw = entity.field_5982;
            var lastPitch = entity.field_6004;
            ...
            if (this.widget.displayMode == DisplayMode.FIXED) {
                ...
                var cursor4x4Buffer = graphics.method_51448().get4x4(new float[16]);
                ...
                switch (widget.displayMode) {
                    case CURSOR -> {
                        var center = new Vector4f(0, entity.method_18381(entity.method_18376()), 0, 1);
                        ...
                        if (entity instanceof class_1309 living) living.field_6259 = -yRotation * 3;

                        entity.field_5982 = -yRotation * .65f;
                        entity.field_6004 = xRotation * 2.5f;
                    }
                    case VANILLA -> {
                        var center = new Vector4f(0, entity.method_17682() / 2, 0, 1);

                        xRotation = (float) Math.atan(localCursorPos.y - center.y) * -20f;
                        yRotation = (float) Math.atan(localCursorPos.x - center.x) * 20f;
                        if (entity instanceof class_1309 living) living.field_6259 = -yRotation;

                        entity.field_5982 = -yRotation;
                        entity.field_6004 = xRotation;
                    }
                }
            }
            ...
            entityTransform.rotate(class_7833.field_40714.rotationDegrees(xRotation));
            entityTransform.rotate(class_7833.field_40716.rotationDegrees(yRotation));

            var entityState = this.host().client().method_1561().method_72977(this.widget.entity, 0);

            if (!this.widget.showNametag) {
                entityState.field_53337 = null;
            }

            graphics.field_59826.method_70922(new BraidEntityElement(
                entityState,
            ...
            if (entity instanceof class_1309 living) living.field_6259 = lastHeadYaw;
            entity.field_6004 = lastPitch;
            entity.field_5982 = lastYaw;
        }
    }
