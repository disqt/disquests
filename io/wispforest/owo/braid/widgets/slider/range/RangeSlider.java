...
public class RangeSlider extends StatefulWidget {
    ...
    public static class State extends WidgetState<RangeSlider> {
        ...
        public Widget build(BuildContext context) {
            ...
            var confirmSound = effectiveStyle.confirmSound() != null ? effectiveStyle.confirmSound().orElse(null) : class_3417.field_15015.comp_349();
        ...
        protected double normalizedValueAt(Constraints constraints, double x, double y, @Nullable Handle grabbedHandle) {
            ...
            if (grabbedHandle == Handle.MAX) {
                ...
                return class_3532.method_15350((coordinate - this.maxHandleSize * 1.5) / denom, 0, 1);
            } else {
                var denom = Math.max(1, constraints.maxFiniteOrMinOnAxis(widget.axis) - this.minHandleSize * 2);
                return class_3532.method_15350((coordinate - this.minHandleSize / 2) / denom, 0, 1);
            }
        }
        ...
        protected double setAbsolute(Constraints constraints, double x, double y) {
            ...
            if (this.grabbedHandle == Handle.MIN) {
                ...
                newNormalizedMin = class_3532.method_15350(normalizedValue, Math.max(0, lower), Math.max(0, upper));
                ...
                newNormalizedMax = class_3532.method_15350(normalizedValue, Math.min(1, lower), Math.min(1, upper));
            }
        ...
        protected void move(Constraints constraints, double dx, double dy) {
            ...
            switch (this.grabbedHandle) {
                case MIN -> {
                    ...
                    var newMin = class_3532.method_15350(this.dragValue, Math.max(0, minCap), Math.max(0, maxCap));
                ...
                case MAX -> {
                    ...
                    var newMax = class_3532.method_15350(this.dragValue, Math.min(1, minCap), Math.min(1, maxCap));
                    this.applyValue(this.normalizedMin, newMax);
                }
                case BOTH -> {
                    var width = this.dragWidth;
                    this.dragValue = class_3532.method_15350(this.dragValue, 0, 1 - width);
        ...
        protected void increment(double increment) {
            ...
            if (target == Handle.MIN) {
                ...
                newMin = class_3532.method_15350(newMin + delta, Math.max(0, lower), Math.max(0, upper));
            } else if (target == Handle.MAX) {
                var lower = newMin + minRangeNorm;
                var upper = maxRangeNorm >= 0 ? newMin + maxRangeNorm : 1;
                newMax = class_3532.method_15350(newMax + delta, Math.min(1, lower), Math.min(1, upper));
            } else {
                newMin = class_3532.method_15350(newMin + delta, 0, 1);
                newMax = class_3532.method_15350(newMax + delta, 0, 1);
            }
            this.applyValue(newMin, newMax);
        }
        ...
        protected void applyValue(double newNormalizedMin, double newNormalizedMax) {
            ...
            newMinValue = class_3532.method_15350(newMinValue, widget.min, widget.max);
            newMaxValue = class_3532.method_15350(newMaxValue, widget.min, widget.max);
