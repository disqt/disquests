...
public class ComboBox<T> extends StatefulWidget {
    ...
    public static final class_2960 ACTIVE_TEXTURE = Owo.id("braid_combobox/active");
    public static final class_2960 HOVERED_TEXTURE = Owo.id("braid_combobox/hovered");
    public static final class_2960 DISABLED_TEXTURE = Owo.id("braid_combobox/disabled");

    // ---

    public final Function<T, class_2561> optionToName;

    public final List<T> options;
    public final @Nullable T selectedOption;
    public final SelectCallback<T> onSelect;

    public ComboBox(Function<T, class_2561> optionToName, List<T> options, @Nullable T selectedOption, SelectCallback<T> onSelect) {
    ...
    public ComboBox(List<T> options, @Nullable T selectedOption, SelectCallback<T> onSelect) {
        this(option -> class_2561.method_43470(Objects.toString(option)), options, selectedOption, onSelect);
    }
    ...
    public List<class_2561> optionNames() {
        return this.options.stream().map(this::nameOption).toList();
    }

    public class_2561 nameOption(@Nullable T option) {
        return option != null
    ...
    private static class State<T> extends WidgetState<ComboBox<T>> {
        ...
        private void textListener() {
            ...
            if (this.widget().optionNames().stream().map(class_2561::getString).anyMatch(s -> s.equals(this.controller.value().text()))) {
                return;
            }
