 ...
 * via {@link #createWithCustomModel(class_2960, ConfigWrapper, class_437)}
...
public class ConfigScreen extends BaseUIModelScreen<FlowLayout> {
    ...
    public static final class_2960 DEFAULT_MODEL_ID = Owo.id("config");
    ...
    protected void build(FlowLayout rootComponent) {
        ...
        rootComponent.childById(LabelComponent.class, "title").text(class_2561.method_43471("text.config." + this.config.name() + ".title"));
        ...
        var sections = new LinkedHashMap<UIComponent, class_2561>();
        ...
        rootComponent.childById(TextBoxComponent.class, "search-field").<TextBoxComponent>configure(searchField -> {
            ...
            var searchHint = class_1074.method_4662("text.owo.config.search");
            ...
            searchField.onChanged().subscribe(s -> {
                ...
                if (!s.equals(this.lastSearchFieldText)) {
                    searchField.method_1868(TextBoxComponent.field_32196);
                    matchIndicator.text(class_2561.method_43473());
                }
            });

            searchField.keyPress().subscribe((input) -> {
                if (!input.method_74230()) return false;

                var query = searchField.method_1882().toLowerCase(Locale.ROOT);
                ...
                if (this.currentMatches.matches.isEmpty()) {
                    matchIndicator.text(class_2561.method_43471("text.owo.config.search.no_matches"));
                    searchField.method_1868(0xEB1D36);
                } else {
                    matchIndicator.text(class_2561.method_43469("text.owo.config.search.matches", this.currentMatchIndex + 1, this.currentMatches.matches.size()));
                    searchField.method_1868(0x28FFBF);
        ...
        this.config.forEachOption(option -> {
            ...
            var container = containers.getOrDefault(
                ...
                UIContainers.collapsible(
                    ...
                    class_2561.method_43471("text.config." + this.config.name() + ".category." + parentKey.asString()),
                    expanded
                    ...
                    if (class_1074.method_4663(categoryKey + ".tooltip")) {
                        nestedContainer.titleLayout().tooltip(class_2561.method_43471(categoryKey + ".tooltip"));
                    }

                    nestedContainer.titleLayout().child(new SearchAnchorComponent(
                        nestedContainer.titleLayout(),
                        option.key(),
                        () -> class_1074.method_4662(categoryKey)
            ...
            if (option.detached()) {
                result.baseComponent().tooltip(
                    this.field_22787.field_1772.method_1728(class_2561.method_43471("text.owo.config.managed_by_server"), Integer.MAX_VALUE)
                        .stream().map(class_5684::method_32662).toList()
                );
            } else {
                var tooltipText = new ArrayList<class_5481>();
                var tooltipTranslationKey = option.translationKey() + ".tooltip";

                if (class_1074.method_4663(tooltipTranslationKey)) {
                    tooltipText.addAll(this.field_22787.field_1772.method_1728(class_2561.method_43471(tooltipTranslationKey), Integer.MAX_VALUE));
                }

                if (option.backingField().hasAnnotation(RestartRequired.class)) {
                    tooltipText.add(class_2561.method_43471("text.owo.config.applies_after_restart").method_30937());
                }
        ...
        if (!sections.isEmpty()) {
            ...
            sections.forEach((component, text) -> {
                var hoveredText = text.method_27661().method_27692(class_124.field_1054);
            ...
            var closeButton = UIComponents.label(class_2561.method_43470("<").method_27692(class_124.field_1067));
            closeButton.tooltip(class_2561.method_43471("text.owo.config.sections_tooltip"));
            ...
            panelContainer.mouseDown().subscribe((click, doubled) -> {
                if (click.comp_4798() < panelContainer.width() - 10) return false;
                ...
                closeButton.text(class_2561.method_43470(closeButton.text().getString().equals(">") ? "<" : ">").method_27692(class_124.field_1067));
    ...
    protected void appendSection(Map<UIComponent, class_2561> sections, Field field, FlowLayout container) {
        ...
        header.childById(LabelComponent.class, "header").<LabelComponent>configure(label -> {
            label.text(class_2561.method_43471(translationKey).method_27695(class_124.field_1054, class_124.field_1067));
            header.child(new SearchAnchorComponent(header, Option.Key.ROOT, () -> label.text().getString()));
        });

        sections.put(header, class_2561.method_43471(translationKey));
    ...
    public boolean method_25404(class_11908 input) {
        if (input.comp_4795() == GLFW.GLFW_KEY_F && input.method_74240()) {
    ...
    public void method_25419() {
        ...
        this.field_22787.method_1507(shouldRestart.booleanValue() ? new RestartRequiredScreen(this.parent) : this.parent);
    }
