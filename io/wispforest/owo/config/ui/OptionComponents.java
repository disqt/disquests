...
public class OptionComponents {
    ...
    public static <T> OptionComponentFactory.Result<FlowLayout, ConfigTextBox> createTextBox(UIModel model, Option<T> option, Function<T, String> toStringFunction, Consumer<ConfigTextBox> processor) {
        ...
        if (option.detached()) {
            ...
            resetButton.onPress(button -> {
                valueBox.method_1852(toStringFunction.apply(option.defaultValue()));
                button.field_22763 = false;
            });

            valueBox.onChanged().subscribe(s -> resetButton.field_22763 = !s.equals(toStringFunction.apply(option.defaultValue())));
        }
    ...
    public static OptionComponentFactory.Result<FlowLayout, OptionValueProvider> createRangeControls(UIModel model, Option<? extends Number> option, int decimalPlaces) {
        ...
        if (option.detached()) {
            ...
            resetButton.field_22763 = (withDecimals ? value.doubleValue() : Math.round(value.doubleValue())) != option.defaultValue().doubleValue();
            resetButton.onPress(button -> {
            ...
            sliderInput.onChanged().subscribe(newValue -> {
                resetButton.field_22763 = (withDecimals ? newValue : Math.round(newValue)) != option.defaultValue().doubleValue();
            });
        }
        ...
        toggleButton.onPress(button -> {
            ...
            button.tooltip(textMode.isTrue()
                    ? class_2561.method_43471("text.owo.config.button.range.edit_with_slider")
                    : class_2561.method_43471("text.owo.config.button.range.edit_as_text")
            );
        });
        ...
        optionComponent.child(new SearchAnchorComponent(
                ...
                option.key(),
                () -> optionComponent.childById(LabelComponent.class, "option-name").text().getString(),
                () -> textMode.isTrue() ? textInput.method_1882() : sliderInput.method_25369().getString()
        ));
    ...
    public static OptionComponentFactory.Result<FlowLayout, ConfigToggleButton> createToggleButton(UIModel model, Option<Boolean> option) {
        ...
        if (option.detached()) {
            ...
            toggleButton.onPress(button -> resetButton.field_22763 = toggleButton.parsedValue() != option.defaultValue());
        }
    ...
    public static OptionComponentFactory.Result<FlowLayout, ConfigEnumButton> createEnumButton(UIModel model, Option<? extends Enum<?>> option) {
        ...
        if (option.detached()) {
            ...
            enumButton.onPress(button -> resetButton.field_22763 = enumButton.parsedValue() != option.defaultValue());
        }
        ...
        optionComponent.child(new SearchAnchorComponent(
                ...
                option.key(),
                () -> optionComponent.childById(LabelComponent.class, "option-name").text().getString(),
                () -> enumButton.method_25369().getString()
        ));
