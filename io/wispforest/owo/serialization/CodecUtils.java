...
public class CodecUtils {
    ...
    public static <T> Endec<T> toEndecWithRegistries(Codec<T> codec, class_9139<class_9129, T> packetCodec) {
        ...
        return Endec.of(
            (ctx, serializer, value) -> {
                if (serializer instanceof ByteBufSerializer<?>) {
                    var buffer = new class_9129(new class_2540(Unpooled.buffer()), ctx.requireAttributeValue(RegistriesAttribute.REGISTRIES).registryAccess());
    ...
    //
    // glisco, 28.04.2024
    public static <B extends class_2540, T> class_9139<B, T> toPacketCodec(Endec<T> endec) {
        return new class_9139<>() {
            @Override
            public T decode(B buf) {
                var ctx = buf instanceof class_9129 registryByteBuf
                    ? SerializationContext.attributes(RegistriesAttribute.of(registryByteBuf.method_56349()))
    ...
    private static DynamicOps<EdmElement<?>> createEdmOps(SerializationContext ctx) {
        ...
        if (ctx.hasAttribute(RegistriesAttribute.REGISTRIES)) {
            ops = class_6903.method_40414(ops, ctx.getAttributeValue(RegistriesAttribute.REGISTRIES).infoGetter());
        }
    ...
    private static <T> DynamicOps<T> unpackOps(DynamicOps<T> ops) {
        var rootOps = ops;
        while (rootOps instanceof class_5379<T>) rootOps = ((DelegatingOpsAccessor<T>) rootOps).owo$delegate();
        return rootOps;
    }
    ...
    static {
        registerCodecAdapter(new CodecAdapter<class_2520, NbtSerializer, NbtDeserializer>() {
            @Override
            ...
            public class_2520 unpackMapLike(MapLike<class_2520> mapLike) {
                ...
                mapLike.entries().forEach(pairs -> {
                    ...
                    if (!(key instanceof class_2519 primitive)) {
                        throw new IllegalStateException("Unable to parse key: " + key);
                    }

                    compound.method_10566(primitive.method_68658().get(), value);
                });
            ...
            public RecordBuilder<class_2520> addToBuilder(class_2520 value, RecordBuilder<class_2520> builder) {
                if (!(value instanceof class_2487 compoundTag)) {
                ...
                for (var key : compoundTag.method_10541()) {
                    result = result.add(key, compoundTag.method_10580(key));
                }
            ...
            public void encodeStruct(SerializationContext ctx, NbtSerializer serializer, Serializer.Struct struct, class_2520 value) {
                ...
                compoundTag.method_10541().forEach(key -> struct.field(key, ctx, NbtEndec.ELEMENT, compoundTag.method_10580(key)));
            }
