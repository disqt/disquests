...
public final class NonNullListEndec {
    ...
    public static <T> Endec<class_2371<T>> forSize(Endec<T> elementEndec, T defaultValue, int size) {
        return forSize(elementEndec, defaultValue, element -> Objects.equals(defaultValue, element), size);
    }

    public static <T> Endec<class_2371<T>> forSize(Endec<T> elementEndec, T defaultValue, Predicate<T> skipWhen, int size) {
        ...
        return entryEndec.listOf().xmap(
                entries -> {
                    var list = class_2371.method_10213(size, defaultValue);
