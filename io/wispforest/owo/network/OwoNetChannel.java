...
public class OwoNetChannel {
    ...
    final class_8710.class_9154<MessagePayload> packetId;
    ...
    private OwoNetChannel(class_2960 id, String ownerClassName, boolean required) {
        ...
        this.packetId = new class_8710.class_9154<>(id);
        ...
        if (FabricLoader.getInstance().getEnvironmentType() == EnvType.CLIENT) {
            ClientPlayNetworking.registerGlobalReceiver(this.packetId, (payload, context) -> {
                clientHandlers.get(endecsByClass.get(payload.message.getClass()).clientHandlerIndex).handle(payload.message, new ClientAccess(context.player().field_3944));
            });
        }
     ...
     * @see #serverHandle(class_1657)
     * @see #serverHandle(MinecraftServer)
     * @see #serverHandle(class_3218, class_2338)
     */
    ...
    public boolean canSendToPlayer(class_3222 player) {
        return canSendToPlayer(player.field_13987);
    }
    ...
    public boolean canSendToPlayer(class_3244 networkHandler) {
        ...
        return OwoHandshake.isValidClient() ?
                getChannelSet(((ServerCommonPacketListenerImplAccessor) networkHandler).owo$getConnection()).contains(this.packetId.comp_2242())
    ...
    public boolean canSendToServer() {
        ...
        return OwoHandshake.isValidClient() ?
                getChannelSet(class_310.method_1551().method_1562().method_48296()).contains(this.packetId.comp_2242())
    ...
    public ServerHandle serverHandle(class_1657 player) {
        if (!(player instanceof class_3222 serverPlayer)) throw new NetworkException("'player' must be a 'ServerPlayerEntity'");
    ...
    public ServerHandle serverHandle(class_2586 entity) {
        if (entity.method_10997().method_8608()) throw new NetworkException("Server handle cannot be obtained on the client");
    ...
    public interface EnvironmentAccess<P extends class_1657, R, N> {
         ...
         * either a {@link MinecraftServer} or a {@link net.minecraft.class_310}
         */
        R runtime();

        /**
         * @return The network handler of the player or client that received the packet,
         * either a {@link net.minecraft.class_634} or a
         * {@link net.minecraft.class_3244}
         */
        N packetListener();
    }
    ...
    record MessagePayload(class_8710.class_9154<MessagePayload> id, Record message) implements class_8710 {
        @Override
